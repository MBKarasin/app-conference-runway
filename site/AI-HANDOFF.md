...
The page header shows one **Updated** timestamp: the time of the last nightly source read (or, when that is unavailable, the latest data build). Manual reviews are dated per record (`reviewed_on` in `sources/overrides.json`). As of 2026-09-22 those reviews were carried out by the AI assistant under the curator's direction. The curator personally confirmed two dates that appear only in images: ENRS 2027 and PNAA 2027. Since 2026-09-24 (curator decision) **fidelity is presumed**: a record that passed its check carries no label at all. The positive labels "Start found" and "Source reviewed" were retired the same day, because labelling the normal case told a reader nothing and buried the exceptions. A label appears only where a check has something to say: **Expected month** (a projection, no date published), **Save the date** (the organizer published only a save-the-date), **Set by rule** (computed from a published observance rule, not read from a page), **Recorded when published** (a past edition, frozen and not re-read), or an exception. The record dialog and the Directory still show exceptions only. Two labels were retired on 2026-09-24 by curator decision: "Source wording changed", because the site never displayed the original excerpt so a reader had no reference point, and the "Organizer dates conflict" card badge, because an organizer ambiguity is only relevant to someone weighing that record, so it now appears inside the record instead. An exception is a record whose latest automated check did not confirm it and has no curator evidence on file; it shows a note ("Date needs review" or "Source not re-checked"), and "Needs review only" in Filters lists just those (deep link `#review=1`). Each record links its source and quotes its wording.
